  ...
  <section className="py-16 sm:py-24">
    <div className="max-w-6xl mx-auto px-5 sm:px-8">
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-10 sm:mb-14">
        <span className="inline-block bg-[#A169A2]/10 text-[#A169A2] font-semibold text-[10px] uppercase tracking-widest px-3 py-1 rounded-full mb-3" style={{ fontFamily: "'DM Sans', sans-serif" }}>Testimonios</span>
          ...
          {testimonios.map((t, i) => (
            <motion.div key={t.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.08 }}
              ...
              <div className="flex items-center gap-3 pt-3 border-t border-gray-100">
                ...
                {t.emprendimiento && (
                  <a href={t.emprendimiento} target="_blank" rel="noopener noreferrer" className="flex-shrink-0 text-[#4CA7C0] hover:text-[#A169A2]"><ExternalLink className="w-4 h-4" /></a>
                )}
              </div>
            </motion.div>
          ))}
