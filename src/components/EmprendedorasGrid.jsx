  ...
  return (
    <section className="py-12 sm:py-24">
      ...
      <div className="pl-4 sm:px-6 sm:container sm:mx-auto sm:max-w-6xl">
        <div className="flex gap-3 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide sm:grid sm:grid-cols-2 lg:grid-cols-4 sm:gap-5 sm:overflow-visible sm:pb-0 pr-4 sm:pr-0">
          {items.map((item, i) => (
            <motion.div
              key={item.id}
              ...
              className="group flex-shrink-0 w-[65vw] sm:w-auto snap-start bg-white rounded-2xl p-4 sm:p-6 text-center shadow-sm hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 border border-transparent hover:border-[#A169A2]/20"
            >
              <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full mx-auto mb-3 sm:mb-4 overflow-hidden ring-2 ring-[#F5EEF8] group-hover:ring-[#A169A2]/30 transition-all">
              ...
              {item.ciudad && (
                <div className="flex items-center justify-center gap-1 text-[9px] sm:text-[10px] text-[#8A7E96] mb-2 sm:mb-3" style={{ fontFamily: "'DM Sans', sans-serif" }}>
              ...
              {item.emprendimiento && (
                <a
                  ...
                  className="inline-flex items-center gap-1 text-[11px] sm:text-xs font-semibold text-[#4CA7C0] hover:text-[#A169A2] transition-colors"
      ...
      {showCTA && (
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.3 }} className="text-center mt-6 sm:mt-14">
          <Link to="/comunidad">
            <Button variant="ghost" size="lg" className="text-[#A169A2] hover:bg-[#A169A2]/5 rounded-full px-6 group text-sm" style={{ fontFamily: "'DM Sans', sans-serif" }}>
